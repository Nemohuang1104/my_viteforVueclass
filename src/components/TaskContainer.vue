<!-- vscode snippets 打vbase->tab 快捷鍵-->
<template>
  <article class="task_container">
    ...
    <div class="task_add_block " ref="taskAddBlock">
      <input type="text" class="task_name" placeholder="輸入待辦事項…" @focus="toggleShadow" @blur="toggleShadow"
        v-model.trim="taskText" @keyup.enter="taskAdd">
      <!-- trim 可以刪除多餘的空格;@keyup.enter 指按enter也會觸發-->
    ...
    <div class="task_list_parent">
      ...
      <ul class="task_list" ref="taskList">
        ...
        <TaskItem :tasks="tasks" @task-edit="taskEdit" @task-star="taskStar" @task-swap="taskSwap" @task-remove="taskRemove"></TaskItem>
  ...
  methods: {
    ...
    taskClear() {
      ...
      if (r) {
        ...
        setTimeout(() => {
          //console.log(this);
          this.tasks = [];
          localStorage.clear(); //資料全刪
          // localStorage.removeItem("tasks");//刪除特定的key
        }, 1000);
    ...
    taskRemove(e,i){
      ...
      if(r){
        ...
        setTimeout(()=>{
          this.tasks.splice(i,1)//陣列資料 記得要加S
        },1000);//一秒鐘之後淡出效果
      }
    }
  }
